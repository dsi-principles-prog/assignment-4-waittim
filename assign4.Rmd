...
get_time_point <- function(df, time_column){
  ...
  df <- df %>%
    mutate_at(time_column, list(time_year=year, time_month=month, time_day=day, time_hour=hour, time_minute=minute, time_second=second)) 
  df
}
```
...
count_rows <- function(df, chr_column){
  ...
  df %>%
    mutate(chr_row_num = str_count(df[[chr_column]], pattern = fixed("\\n")) + 1)
  df
}
```
...
count_pattern <- function(df, tag_column){
  ...
  df %>%
    mutate(pattern_num = str_count(df[[tag_column]], pattern = fixed('"|"')) + 1)
  df
}
```
...
get_chr_length <- function(df, chr_column){
  ...
  df %>%
    mutate(chr_length = stringr::str_length(df[[chr_column]]))
  df
}
```
